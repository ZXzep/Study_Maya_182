...
ctl.setParent(None)
jntInvMat = jnt.getAttr('worldInverseMatrix')
jntInvTransMat = pm.datatypes.TransformationMatrix(jntInvMat)
ctl.setTranslation(jntInvTransMat.getTranslation(space='world'))
ctl.setRotation(jntInvTransMat.getRotation())
# move pivot point to origin
pm.move(0,0,0,ctl.scalePivot,ctl.rotatePivot)
# Freeze  ctl's Transformation
pm.makeIdentity(ctl,apply=True)
# Refresh fix bug in maya2022
pm.refresh()
